...
void myCG(int n, double* vrr, int* colind, int* rbegin, double *b_loc, double*x_loc,  int rank, int nproc, int err_size){
...
int main(int argc, char const *argv[]){
  ...
  if( argc!=5 ){
    if(rank==ROOT_ID)
      fprintf(stderr,  ACRED"usage: ""%s "ACGREEN"<inA.mtx> <inb.mtx>"ACRED" <outx.mtx>"ACGREEN" <ErrSize> "ACRESET"\n", argv[0]);
    MPI_Finalize();
    exit(1);
  ...
  if(rank==ROOT_ID){
    ...
    err_size= atoi(argv[4]);
  }
  MPI_Bcast(&err_size, 1, MPI_INT, ROOT_ID, MPI_COMM_WORLD);
  ...
  mp = m/nproc;
  if(err_size>mp) { fprintf(stdout, "err_size(%d)>mp(%d) that's not right\n", err_size, mp);  exit(4);}
  if(m!=mrhs) exit(3);
  ...
  fprintf(stdout,"finish reading files, and start the CG procedure\n");
  myCG(m, avrr, colind, rbegin, b_loc, x_loc, rank, nproc, err_size);
